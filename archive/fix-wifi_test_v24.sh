#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# File: fix-wifi_test_v23.sh (FULLY SAFE + NO LINGERING PROCESSES)
# -----------------------------------------------------------------------------
...
# Prevent leftover processes from prior runs
pkill -f "journalctl -f -n 0" 2>/dev/null || true
pkill -f "dmesg -w" 2>/dev/null || true
